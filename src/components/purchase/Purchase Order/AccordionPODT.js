...
function AccordionPODT({ accDocNo, onSaveSuccess, refreshPOData, ocrLineItems = [] }) {
  ...
  const handleEditDetail = (product) => {
    ...
    setItemToEdit({
      accDocNo,
      accItemNo: product.AccItemNo,
      docStatus,
      accDocType,
      selectedDocConfigID: location.state?.selectedDocConfigID || null,
      AccSourceDocNo: product.AccSourceDocNo || "",
      AccSourceDocItem: product.AccSourceDocItem || 0,
      StockTransNo: product.StockTransNo || 0,
    });
      ...
      {showEditDetailModal && itemToEdit && (
        <>
          <AccordionPOEditDT
            ...
            qty={itemToEdit.qty}
            accSourceDocNo={itemToEdit.AccSourceDocNo}
            accSourceDocItem={itemToEdit.AccSourceDocItem}
            stockTransNo={itemToEdit.StockTransNo}
          />
        </>
      )}
