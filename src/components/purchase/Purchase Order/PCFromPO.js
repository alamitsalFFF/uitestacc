...
//จ่ายเช็ค
export const PCfromPO = async (AccDocNoC, bankacc, bankName, transtype, rcpno, duedate, webAddress, navigate) => {
  ...
  try {
    ...
    const PCfromPO = {
      name: "Insert_PCFromPO", //ยังไม่สมบรูณ์
      parameters: [
        {
          param: "userid",
    ...
    console.log("PCFromPO:", PCfromPO)
    const responsePCFromPO = await axios.post(
    ...
    if (responsePCFromPO.status === 200) {
      console.log("responsePCFromPO:", responsePCFromPO);
      const pcNo = responsePCFromPO.data.data[0].PCNo;
      ...
      return {
        ...
        raw: responsePCFromPO.data,
      };
    } else {
      // non-200 — return raw for inspection
      return { raw: responsePCFromPO.data || responsePCFromPO };
    }
  } catch (error) {
