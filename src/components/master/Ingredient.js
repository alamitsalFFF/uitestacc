...
function Ingredient() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: `ProductCode ห้ามช้ำกัน!!`,
        text: `กรุณาแก้ ProductCode:${formDataToSend.productCode}`,
      });
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchProductTypeOptions = async () => {
      try {
        const response = await axios.get(
          `${API_BASE}/ProductType/GetProductType?isMaterial=false&isService=false`
        );
  ...
  const handleProductTypeSelect = (productTypeCode, assetAccCode, incomeAccCode, expenseAccCode) => {
    setFormData({
      ...formData, productTypeCode: productTypeCode,
  ...
  return (
    <div className="row" style={{ padding: "5%" }}>
      ...
      <Modal open={openModal} onClose={handleCloseModal}>
        ...
        >
          <List>
            <h4 style={{ textAlign: "center" }}>Select IngredientType</h4>
            <Divider
              variant="middle"
              component="li"
            ...
            {getPaginatedData().map((producttype) => (
              <ListItem key={producttype.producttypeID} disablePadding>
                {/* <ListItemButton
                  onClick={() =>
                    handleProductTypeSelect(producttype.productTypeCode)
                  }
                > */}
                <ListItemButton
                  onClick={() =>
