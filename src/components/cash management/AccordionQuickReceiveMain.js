...
function AccordionQuickReceiveMain({ onSaveSuccess }) {
  ...
  const handleSubmit = async (event) => {
    ...
    // ข้ามการตรวจ validity ของฟิลด์ inccode (name="inccode") และ acccode (name="acccode")
    const incEl = form.querySelector('[name="inccode"], #inccode');
    ...
    const formData = {
      name: "Insert_QuickReceive",
      parameters: [
        {
          param: "inccode",
          value: event.target.inccode.value,
        },
        {
          param: "curr",
        ...
        {
          param: "bankcode",
          value: event.target.bankcode.value,
        },
        {
          param: "bankname",
  ...
  // selected value for INCCODE (income acccode)
  const [selectedIncCode, setSelectedIncCode] = useState({
    code: "",
    name: "",
  });
  const [openAccCodeModal, setOpenAccCodeModal] = useState(false); // สถานะการเปิด/ปิด Modal AccCode
  const [accCodeModalFilter, setAccCodeModalFilter] = useState(null); // null = no filter, 4 = income only
  // which target field the modal should set: "primary" (acccode) or "inc" (inccode)
  ...
  const handleOpenAccCodeModal = (target = "primary") => {
    ...
    setAccCodeModalFilter(target === "inc" ? 4 : null);
  ...
  const handleConfirmAccCodeSelection = (accCodeData) => {
    ...
    if (target === "inc") {
      // ถ้ามี category และไม่ใช่ 4 ให้บล็อก
      if (cat != null && Number(cat) !== 4) {
        Swal.fire({
          icon: "warning",
          title: "เลือกไม่ได้",
          text: "กรุณาเลือกบัญชีหมวดประเภทรายได้ (หมวด 4) สำหรับ INCCODE",
        });
        handleCloseAccCodeModal();
        return;
      }
      // ถ้าไม่มี category หรือเป็น 4 ให้ยอมรับ
      setSelectedIncCode(accCodeData);
    } else {
  ...
  const transtype = [
    ...
    { value: "CHQ", label: "CHQ" },
    { value: "CRCARD", label: "CRCARD" },
    { value: "DRCARD", label: "DRCARD" },
  ];
