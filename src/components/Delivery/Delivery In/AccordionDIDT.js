...
import AccordiionDIAddDT from "./AccordiionDIAddDT.js";
import AccordionSelectProductDI from "./AccordionSelectProductDI.js";
import AccordionDIEditDT from "./AccordionDIEditDT.js";
import DIManagementComponentFromPO from "./DIManagementComponentFromPO.js";

function AccordionDIDT({ accDocNo, onSaveSuccess }) {
      ...
      (async () => {
        try {
          ...
          if (responseH.status === 200 && responseD.status === 200) {
            setLoading(false);
            console.log("vDI_H:", responseH.data);
            console.log("vDI_D:", responseD.data);
            setDIH(responseH.data);
            setDID(responseD.data.sort((a, b) => a.AccItemNo - b.AccItemNo));
          } else {
            setLoading(false);
      ...
      (async () => {
        try {
          ...
          if (responseH.status === 200 && responseD.status === 200) {
            ...
            console.log("Sorted vDI_D:", sortedData);
            setDIH(responseH.data);
            setDID(sortedData);
          } else {
            setLoading(false);
  ...
  const handleChoosePO = () => {
    ...
    setShowDIManagement(true); // ตั้งค่าให้แสดง DIManagementComponentFromPO
  };
  const handleCloseDIManagement = () => {
    setShowDIManagement(false);
  };

  const fetchDataPODetails = () => {
    if (accDocNo) {
      // ดึงข้อมูล DI_H และ DI_D ด้วย accDocNo จาก props
      const vDI_H = {
        viewName: "vDI_H",
        parameters: [
      ...
      (async () => {
        try {
          setLoading(true);
          const responseH = await axios.post(`${API_VIEW_RESULT}`, vDI_H, {
            headers: { "Content-Type": "application/json" },
          });

          const responseD = await axios.post(`${API_VIEW_RESULT}`, vDI_D, {
            headers: { "Content-Type": "application/json" },
          });
  ...
  const handleEditDetail = (AccItemNo) => {
    const docStatus = dih[0]?.DocStatus;
    const accDocType = dih[0]?.AccDocType;
    setItemToEdit({
      accDocNo,
      accItemNo: AccItemNo,
      docStatus,
  ...
  }, [fetchDataPODetails, handleCloseEditDetailModal]);
    ...
    <div>
      <div className="row">
        ...
        >
          <p>
            Date:
            {dih && dih.length > 0 && (
              <FormatDate dateString={dih[0].AccEffectiveDate} />
            )}
          </p>
        </ListItem>
      </div>
      <div className="row">
        <ListItem style={{ display: "flex", alignItems: "center" }}>
          <h5 style={{ color: "blue" }}>
            &nbsp;{dih && dih.length > 0 && dih[0].PartyName}
          </h5>
        </ListItem>
      </div>
      ...
      {/* {(!dih || dih.length === 0 || dih[0]?.DocStatus === 0) && ( */}
      {(!dih || dih.length === 0 || (dih[0]?.DocStatus === 0 || dih[0]?.DocStatus === 1)) && (
        <>
          <Divider
            variant="middle"
            component="li"
          ...
          >
            ...
            {showDIManagement && ( //เพิ่มให้เลิอกChoosePOมาทDI 
              <DIManagementComponentFromPO
                currentDINo={dih[0].AccDocNo}
                onClose={handleCloseDIManagement}
                onSave={fetchDataPODetails} // ฟังก์ชันรีเฟรชข้อมูล
              />
            )}
          </div>
        </>
