...
export const CancelDI = async (AccDocNoC, navigate) => {
  ...
  if (cancelReason) {
    try {
      ...
      // สร้าง payload สำหรับการยกเลิก DI
      const payload = {
        name: "CancelDI",
        parameters: [
          {
      ...
      console.log("Canceling DI with payload:", payload);
      ...
      if (response.status === 200) {
        const dino = response.data.data[0].DocNo;
        console.log("CancelDI created successfully:", response);
        console.log("CancelDI created successfully:", dino);
        Swal.fire({
          icon: "success",
          title: `ยกเลิกเอกสาร ${dino} เรียบร้อยแล้ว`,
        ...
        console.error("Error creating CancelDI:", response.status, response.statusText);
        Swal.fire({
          icon: "error",
      ...
      console.error("Error creating CancelDI:", error);
      Swal.fire({
        icon: "error",
        title: "เกิดข้อผิดพลาด",
