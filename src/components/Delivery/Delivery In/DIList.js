...
import DriveImagePreview from "../DraftData/Preview";
...
import PreviewData from "../DraftData/PreviewData";
...
function DIHDList() {
  ...
  const fetchRequisitions = useCallback(async () => {
    setLoading(true);
    let parameters = [{ field: "AccDocType", value: "DI" }];
    ...
    try {
      console.log("vDI_H:", vDI_H);
      const response = await axios.post(API_VIEW_RESULT, vDI_H, {
        headers: {
      ...
      if (response.status === 200) {
        setLoading(false);
        console.log("data_vDI_H", response.data);
  ...
  const handleEditClick = (filtered, index) => {
    ...
    navigate(`${URL}AccordionDI?accDocNo=${filtered.AccDocNo}`); // นำทางไปยัง AccordionDI
  };
  ...
  const handleEditClick1 = (filtered, index) => {
    ...
    dispatch(setAccDocType(filtered.AccDocType));
    dispatch(setStatusName(filtered.StatusName));
    navigate(`${URL}DIHeader?accDocNo=${filtered.AccDocNo}`); // นำทางไปยัง DIHeader
  };
  ...
  const buttonActions = [
    ...
    },
    {
      ...
      onClick: handleDIDraftOCR, //OCR พี่ฐากร
    },
    {
      icon: (
    ...
    <div className="row" style={{ padding: "5%", paddingTop: "1px" }}>
      ...
      >
        <Box sx={{
          ...
          <PreviewData
            googleDriveFileId={globalPreviewFileIdDraft} // ส่ง File ID จาก Global State
            accDocType={accDocType} // ส่ง DocType ไปด้วย
