...
/**
 * DocStatusSO Component
 * ดึง StatusName จาก API (vSO_H) โดยใช้ AccDocNo ที่รับเข้ามา
 * และแสดงผลใน TextField
 */
const DocStatusSO = ({ accDocNo, DocType }) => { // เปลี่ยนจาก statusName เป็น accDocNo
  ...
  useEffect(() => {
    const fetchStatusName = async () => {
      // ตรวจสอบว่ามี accDocNo ก่อนเรียก API และ accDocNo ต้องไม่เป็น "SO25xx..." ซึ่งเป็นค่าเริ่มต้นชั่วคราว
      ...
      try {
        const vSO_H_Payload = { // สร้าง payload สำหรับ API request
          viewName: "vSO_H",
          parameters: [
        ...
        const response = await axios.post(API_VIEW_RESULT, vSO_H_Payload, {
