...
import AccordiionSOAddDT from "./AccordiionSOAddDT.js";
import SOManagementComponentFromSR from "./SOManagementComponentFromSR.js";
import AccordionSelectProductSO from "./AccordionSelectProductSO.js";
import Stack from "@mui/material/Stack";
import Button from "@mui/material/Button";
import AccordionSOEditDT from "./AccordionSOEditDT.js";

function AccordionSODT({ accDocNo, onSaveSuccess, refreshPOData }) {
  ...
  useEffect(() => {
    if (accDocNo) {
      ...
      console.log("accDocNo from SOHeader:", accDocNo);
      console.log("accDocNo from SOHeader:", typeof accDocNo);

      const vSO_H = {
        viewName: "vSO_H",
        parameters: [
          // { field: "AccDocType", value: "SO" },
      ...
      (async () => {
        try {
          ...
          if (responseH.status === 200 && responseD.status === 200) {
            setLoading(false);
            console.log("vSO_H:", responseH.data);
            console.log("vSO_D:", responseD.data);

            const sortedData = responseD.data.sort(
              (a, b) => a.AccItemNo - b.AccItemNo
            );
            setSOH(responseH.data);
            setSOD(sortedData);
          } else {
            setLoading(false);
  ...
  const handleEditDetail1 = (AccItemNo) => {
    console.log("DocStatus:", soh[0].DocStatus);
  ...
  const handleChooseSR = () => {
    console.log("AccDocNo:", accDocNo);

    setShowSOManagement(true); // ตั้งค่าให้แสดง SOManagementComponentFromSR
  };
  const handleCloseSOManagement = () => {
    setShowSOManagement(false);
  };

  const fetchDataSRDetails = () => {
    if (accDocNo) {
      const vSO_H = {
        viewName: "vSO_H",
        parameters: [
      ...
      (async () => {
        try {
          setLoading(true);
          const responseH = await axios.post(`${API_VIEW_RESULT}`, vSO_H, {
            headers: { "Content-Type": "application/json" },
          });

          const responseD = await axios.post(`${API_VIEW_RESULT}`, vSO_D, {
            headers: { "Content-Type": "application/json" },
          });
          ...
          if (responseH.status === 200 && responseD.status === 200) {
            setLoading(false);
            setSOH(responseH.data);
            setSOD(responseD.data.sort((a, b) => a.AccItemNo - b.AccItemNo));
          } else {
            setLoading(false);
            console.error("Error fetching data");
          }
  ...
  const handleEditDetail = (AccItemNo) => {
    const docStatus = soh[0]?.DocStatus;
    const accDocType = soh[0]?.AccDocType;
    setItemToEdit({
      accDocNo,
      accItemNo: AccItemNo,
      docStatus,
  ...
  useEffect(() => {
    if (sod && sod.length > 0) {
      const itemNo = sod.map((item) => item.AccItemNo);
      const maxItemNo = Math.max(...itemNo);
      setItemCounter(maxItemNo + 1);
    } else {
      setItemCounter(1);
    ...
    <div>
      ...
      <div className="row">
        ...
        >
          ...
          >
            ...
            >
              Date:&nbsp;
              {soh && soh.length > 0 && soh[0]?.AccEffectiveDate ? (
                <FormatDate dateString={soh[0].AccEffectiveDate} />
              ) : (
      ...
      <div className="row">
        ...
        >
          <h5 style={{ color: "blue" }}>
            &nbsp;
            {soh && soh.length > 0 && soh[0].PartyName
              ? soh[0].PartyName
              : partyName}
          </h5>
        </ListItem>
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: "6px", flexWrap: "wrap" }}>
              ...
              >
                {product.ProductCode}/<Abbreviation textName={product.SalesDescription} />
      ...
      {(!soh || soh.length === 0 || soh[0]?.DocStatus === 0) && (
        <>
          <Divider
            variant="middle"
            component="li"
          ...
          >
            ...
            {showSOManagement && (
              <SOManagementComponentFromSR
                currentSONo={accDocNo}
                // docRefNo={poh.docRefNo}//เช็คอีกครั้งควรเอาไปแท็กไหม
                onClose={handleCloseSOManagement}
                onSave={fetchDataSRDetails} // ฟังก์ชันรีเฟรชข้อมูล
              />
            )}
            {/* </ListItem> */}
          </div>
      ...
      <div className="row">
        {soh.map((productH, index) => (
          ...
          >
            ...
            {/* Row: TotalVat */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 8px" }}>
              <h5 style={{ color: "#1565c0", fontWeight: 500, margin: 0 }}>Total VAT 7%</h5>
              <h2 style={{ margin: 0, color: "#1565c0" }}>{formatNumber(productH.TotalVat)}</h2>
            </div>

            {/* Row: TotalWht */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 8px" }}>
              <h5 style={{ color: "#1565c0", fontWeight: 500, margin: 0 }}>Total WHT</h5>
            ...
            >
              <h5 style={{ color: "#ffffff", fontWeight: 700, margin: 0, letterSpacing: "0.04em" }}>Net Total</h5>
              <h2
                style={{
                  margin: 0,
                  color: "#ffffff",
