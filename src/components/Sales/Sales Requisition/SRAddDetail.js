...
import {useAuthFetch} from "../../Auth/fetchConfig"; // ใช้ hook สำหรับ fetch ข้อมูล

function SRAddDetail() {
  ...
  const [rateVatValue, setRateVatValue] = useState(
    selectedProducts && selectedProducts.length > 0
      ? selectedProducts.map((p) => p.rateVat)
      : []
  );
  const [rateWhtValue, setRateWhtValue] = useState(
    selectedProducts && selectedProducts.length > 0
      ? selectedProducts.map((p) => p.rateWht)
      : []
  );
  const [vatTypeValue, setVatTypeValue] = useState(
    selectedProducts && selectedProducts.length > 0
      ? selectedProducts.map((p) => p.vatType)
      : []
  );
  ...
  const [productSizeUnitValue, setProductSizeUnitValue] = useState(
    selectedProducts && selectedProducts.length > 0
      ? selectedProducts.map((p) => p.ProductSizeUnit)
      : []
  );
  ...
  const saveDataToAPI = async (data) => {
    try {
      const token = localStorage.getItem("token"); // หรือที่เก็บ token ของคุณ
      const response = await authFetch(
        "http://103.225.168.137/apiaccbk2/api/Prototype/AccTransaction/SetAccTransactionDT",
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="row" style={{ padding: "5%", paddingTop: "1px" }}>
      <h1 style={{ textAlign: "center" }}>Add SR Detail</h1>
      {/* <div>&nbsp;</div> */}
      {/* <div>&nbsp;</div> */}
      <div className="row">
