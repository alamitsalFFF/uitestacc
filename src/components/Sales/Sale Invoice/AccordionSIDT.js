...
function AccordionSIDT({ accDocNo, onSaveSuccess, refreshPOData }) {
  ...
  useEffect(() => {
    if (accDocNo) {
      ...
      console.log("accDocNo from siheader:", accDocNo);
      console.log("accDocNo from siheader:", typeof accDocNo);

      const vAR_H = {
        viewName: "vAR_H",
        parameters: [
          // { field: "AccDocType", value: "SI" },
      ...
      (async () => {
        try {
          ...
          if (responseH.status === 200 && responseD.status === 200) {
            setLoading(false);
            console.log("vAR_H:", responseH.data);
            console.log("vAR_D:", responseD.data);

            const sortedData = responseD.data.sort(
              (a, b) => a.AccItemNo - b.AccItemNo
            );
            setSIH(responseH.data);
            setSID(sortedData);
          } else {
            setLoading(false);
  ...
  const fetchDataSRDetails = () => {
    if (accDocNo) {
      ...
      (async () => {
        try {
          setLoading(true);
          const responseH = await axios.post(`${API_VIEW_RESULT}`, vAR_H, {
            headers: { "Content-Type": "application/json" },
          });

          const responseD = await axios.post(`${API_VIEW_RESULT}`, vAR_D, {
            headers: { "Content-Type": "application/json" },
          });
          ...
          if (responseH.status === 200 && responseD.status === 200) {
            setLoading(false);
            setSIH(responseH.data);
            setSID(responseD.data.sort((a, b) => a.AccItemNo - b.AccItemNo));
          } else {
            setLoading(false);
            console.error("Error fetching data");
          }
  ...
  const handleEditDetail = (AccItemNo) => {
    const docStatus = sih[0]?.DocStatus;
    const accDocType = sih[0]?.AccDocType;
    setItemToEdit({
      accDocNo,
      accItemNo: AccItemNo,
      docStatus,
  ...
  useEffect(() => {
    if (sid && sid.length > 0) {
      const itemNo = sid.map((item) => item.AccItemNo);
      const maxItemNo = Math.max(...itemNo);
      setItemCounter(maxItemNo + 1);
    } else {
      setItemCounter(1);
    ...
    <div>
      ...
      <div className="row">
        ...
        >
          ...
          >
            ...
            >
              Date:&nbsp;
              {sih && sih.length > 0 && sih[0]?.AccEffectiveDate ? (
                <FormatDate dateString={sih[0].AccEffectiveDate} />
              ) : (
      ...
      <div className="row">
        ...
        >
          <h5 style={{ color: "blue" }}>
            &nbsp;
            {sih && sih.length > 0 && sih[0].PartyName
              ? sih[0].PartyName
              : partyName}
          </h5>
        </ListItem>
      ...
      {(!sih || sih.length === 0 || sih[0]?.DocStatus === 0) && (
        <>
          <Divider
            variant="middle"
            component="li"
