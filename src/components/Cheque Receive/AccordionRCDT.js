...
function AccordionRCDT({ accDocNo, onSaveSuccess }) {
  ...
  const fetchDataRCDetails = useCallback(() => {
    // useEffect(() => {
    if (accDocNo) {
      ...
      (async () => {
        try {
          ...
          if (responseH.status === 200 && responseD.status === 200) {
            setLoading(false);
            console.log("vRC_H:", responseH.data);
            console.log("vRC_D:", responseD.data);
            setRCH(responseH.data);
            setRCD(responseD.data.sort((a, b) => a.AccItemNo - b.AccItemNo));
          } else {
            setLoading(false);
      ...
      (async () => {
        try {
          ...
          if (responseH.status === 200 && responseD.status === 200) {
            ...
            console.log("Sorted vRC_D:", sortedData);
            setRCH(responseH.data);
            setRCD(sortedData);
          } else {
            setLoading(false);
  ...
  useEffect(() => {
    fetchDataRCDetails();
  }, [fetchDataRCDetails, location.state?.addProducts]);

  const handleEditDetail = (AccItemNo) => {
    const docStatus = rch[0]?.DocStatus;
    const accDocType = rch[0]?.AccDocType;
    setItemToEdit({
      accDocNo,
      accItemNo: AccItemNo,
      docStatus,
  ...
  }, [fetchDataRCDetails, handleCloseEditDetailModal]);

  // คำนวณ itemCounter
  useEffect(() => {
    if (rcd && rcd.length > 0) {
      const itemNo = rcd.map((item) => item.AccItemNo);
      const maxItemNo = Math.max(...itemNo);
      setItemCounter(maxItemNo + 1);
    } else {
      setItemCounter(1);
    ...
    <div>
      ...
      <div className="row">
        ...
        >
          ...
          >
            ...
            >
              Date:&nbsp;
              {rch && rch.length > 0 && rch[0]?.AccBatchDate && (
                <FormatDate dateString={rch[0].AccBatchDate} />
              )}
            </span>
          </div>
        </ListItem>
      ...
      <div className="row">
        ...
        >
          <h5 style={{ color: "blue" }}>
            &nbsp;
            {rch && rch.length > 0 && rch[0].PartyName
              ? rch[0].PartyName
              : partyName}
          </h5>
        </ListItem>
      ...
      {(!rch || rch.length === 0 || rch[0]?.DocStatus === 0) && (
        <>
          <Divider
            variant="middle"
            component="li"
