...
function PCList() {
  ...
  const fetchRequisitions = useCallback(async () => {
    setLoading(true);
    let parameters = [{ field: "AccDocType", value: "PC" }];
    ...
    try {
      console.log("vPC_H:", vPC_H);
      const response = await axios.post(API_VIEW_RESULT, vPC_H, {
        headers: {
      ...
      if (response.status === 200) {
        setLoading(false);
        console.log("data_vPC_H:", response.data);
            ...
            groupedTransactions.map((transaction, index) => (
              ...
              >
                <ListItem style={{ display: "flex", alignItems: "center" }}>
                  <div>
                    <h5
                      style={{ marginTop: "5px", marginLeft: "10px" }} //onClick={() => handleEditClick1(transaction)}
                    >
