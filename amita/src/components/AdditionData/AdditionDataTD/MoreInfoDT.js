...
const MoreInfoDT = ({
  ...
  console.log("MoreInfoDT props:", { open, accDocNo, docConfigID, accItemNo });
  console.log("MoreInfoDT accItemNo:", accItemNo);
  ...
  const fetchSchemaAndData = useCallback(async () => {
    ...
    try {
      // 1. Fetch Schema
      console.log("Fetching schema for docConfigID:", docConfigID);
      console.log("Fetching schema for docConfigID:", typeof docConfigID);
      const schemaResponse = await authFetch(
        `${API_BASE}/DocConfig/GetDocConfigSchema?docConfigID=${docConfigID}&configType=1`,
        {
          headers: {
      ...
      const dataResponse = await authFetch(
        `${API_BASE}/AdditionData/GetAdditionData?docNo=${accDocNo}&seq=${accItemNo}`,
        {
          headers: {
            "Content-Type": "application/json",
            // Authorization: `Bearer ${localStorage.getItem("userToken")}`,
      ...
      setParsedDocSchema([]);
      setMoreInfoData({
        AccDocNo: accDocNo, //, AccDocType: accDocType
      }); // Reset with basic info
      Swal.fire({
        icon: "error",
    ...
    try {
      ...
      const response = await authFetch(
        `${API_BASE}/AdditionData/SetAdditionData`,
        {
          ...
          headers: {
            "Content-Type": "application/json",
            // Authorization: `Bearer ${localStorage.getItem("userToken")}`, // Ensure token is included
          },
    ...
    >
      <Box sx={style}>
        <Typography
          ...
          textAlign="center"
        >
          Detail MoreInfo {accDocNo} Item:{accItemNo}
        </Typography>
        <Box sx={{ mt: 2 }}>
          {/* <TextField
            fullWidth
        ...
        >
          {/* {isEditMode && (
            <Button variant="contained" onClick={handleDocConfigSchema}>
              More Info  
            </Button>
          )} */}
          ...
          {parsedDocSchema.length !== 0 && !isEditMode && (
