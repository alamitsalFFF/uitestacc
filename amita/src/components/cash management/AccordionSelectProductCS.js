...
function AccordionSelectProductCS({
  isOpen,
  onClose,
  onSave,
  accDocNo,
  ...
  const handleProductSelect1 = (product) => {
    // ล้างรายการสินค้าที่เคยเลือกไว้ทั้งหมด
    dispatch(
      setSelectedProducts([
        {
          ...product,
          ...
          checked: true, // ตั้งค่าเป็น true เพราะถูกเลือกแล้ว
        },
      ])
    );
  };
  ...
  const handleProductSelect = (product) => {
    // อัปเดต state โดยแทนที่รายการเดิมด้วยรายการใหม่ที่ถูกเลือก
  ...
  const handleConfirm = () => {
    // ตรวจสอบว่ามีสินค้าถูกเลือกอย่างน้อยหนึ่งรายการ
    if (selectedProducts.length > 0) {
      // ส่งข้อมูลสินค้าที่เลือกกลับไป โดยเลือกรายการแรก
      onSave(selectedProducts[0]);
    }
    // ปิด Modal
    onClose();
  };
                ...
                <ListItem style={{ display: "flex", alignItems: "center" }}>
                  <div>
                    <h5 style={{ marginTop: "5px", marginLeft: "10px" }}>
                      ...
                      {/* ตรวจสอบว่าสินค้าปัจจุบันอยู่ใน array ที่ถูกเลือกหรือไม่ */}
...
export default AccordionSelectProductCS;
