...
function RVEditDetail({ open, onClose, entryId: entryIdProp, seq: seqProp, journalNo: journalNoProp, onSaved }) {
    ...
    try {
      ...
      if (response && response.message === "JournalDT datail updated.") {
        ...
        if (onSaved) {
          ...
          navigate(`${URL}AccordionRV?journalNo=${journalNoFromState}`);
        }
      } else {
  ...
  const handleDelete = async () => {
    try {
      ...
      else navigate(`${URL}AccordionRV?journalNo=${journalNoFromState}`);
    } catch (error) {
