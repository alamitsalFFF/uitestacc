...
function SIList() {
  ...
  const fetchRequisitions = useCallback(async () => {
    ...
    try {
      console.log("v:", vAR_H);
      const response = await axios.post(API_VIEW_RESULT, vAR_H, {
        headers: {
  ...
  const handleEditClick = (filtered, index) => {
    ...
    navigate(`${URL}Accordion${accDocType}?accDocNo=${filtered.AccDocNo}`); // นำทางไปยัง SIHeader
  };
  ...
  return (
    <div className="row" style={{ padding: "5%", paddingTop: "1px" }}>
      {/* <h2 style={{ textAlign: "center", textDecorationLine: "underline" }}
        onClick={handleGoMenu}>&nbsp;{categoryOptions}&nbsp;</h2> */}
      <DocConfigHeader
