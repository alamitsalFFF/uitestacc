...
import DIManagementComponentFromPO from "../Delivery/Delivery In/DIManagementComponentFromPO.js";
import AccordiionDOAddDT from "../Delivery/Delivery Out/AccordiionDOAddDT.js";
import AccordionSelectProductDO from "../Delivery/Delivery Out/AccordionSelectProductDO.js";
import AccordionDOEditDT from "../Delivery/Delivery Out/AccordionDOEditDT.js";
import DOManagementComponentFromSO from "../Delivery/Delivery Out/DOManagementComponentFromSO.js";
import Abbreviations from "../DataFilters/Abbreviations.js";

function TransactionDetail({ accDocNo,onSaveSuccess }) {
  ...
  // useEffect(() => {
    if (accDocNo) {
      ...
      const vTransaction_D = {
        ...
        parameters: [
          { field: "DocNo", value: accDocNo }, // ใช้ accDocNo จาก Redux store
        ],
         results: [
            { sourceField: "DocNo" },
      ...
      (async () => {
        try {
          ...
          if (responseH.status === 200 && responseD.status === 200) {
            ...
            setHD(responseH.data);
            setDT(responseD.data.sort((a, b) => a.AccItemNo - b.AccItemNo));
          } else {
            setLoading(false);
    ...
    if (accDocNo) {
      ...
      (async () => {
        try {
          ...
          if (responseH.status === 200 && responseD.status === 200) {
            setLoading(false);
            console.log("vTransaction_H:", responseH.data);
            console.log("vTransaction_D:", responseD.data);

            const sortedData = responseD.data.sort(
              (a, b) => a.AccItemNo - b.AccItemNo
            );
            console.log("Sorted vTransaction_D:", sortedData);
            setHD(responseH.data);
            setDT(sortedData);
          } else {
            setLoading(false);
    ...
    const fetchDataSODetails = () => {
      if (accDocNo) {
        ...
        (async () => {
          try {
            setLoading(true);
            const responseH = await axios.post(`${API_VIEW_RESULT}`, vTransaction_H, {
              headers: { "Content-Type": "application/json" },
            });

            const responseD = await axios.post(`${API_VIEW_RESULT}`, vTransaction_D, {
              headers: { "Content-Type": "application/json" },
            });
  ...
  // }, [fetchDataDIDetails, location.state?.addProducts]); 
    ...
    const handleEditDetail = (AccItemNo) => {
      const docStatus = hd[0]?.DocStatus;
      const accDocType = hd[0]?.AccDocType;
      setItemToEdit({
        accDocNo,
        accItemNo: AccItemNo,
        docStatus,
   ...
   console.log("handleEditDetail called. Modal state:", true);
    ...
    useEffect(() => {
      if (dt && dt.length > 0) {
        const itemNo = dt.map((item) => item.AccItemNo);
        const maxItemNo = Math.max(...itemNo);
        setItemCounter(maxItemNo + 1);
      } else {
        setItemCounter(1);
  ...
  return (
    <div>
      <div className="row">
        ...
        >
          <p>
            Date:
            {hd && hd.length > 0 && (
              // <FormatDate dateString={hd[0].AccEffectiveDate}/>
              <FormatDate dateString={hd[0].AccBatchDate}/>
            )}
          </p>
        </ListItem>
      </div>
      ...
      {dt.map((product, index) => (
          ...
          <ListItem style={{ display: "flex", alignItems: "center" }}>
            <div>
              <h5>
                ...
                {/* <Abbreviations textName={product.SaleProductCode} />/ */}
       ...
       {/* {(!hd || hd.length === 0 || (hd[0]?.DocStatus === 0 || hd[0]?.DocStatus === 1)) && (
        <>
          <Divider
            variant="middle"
            component="li"
          ...
          >
            ...
            {showDOManagement && ( //เพิ่มให้เลิอกChooseSOมาทDO
              <DOManagementComponentFromSO
                currentDONo={hd[0].AccDocNo}
